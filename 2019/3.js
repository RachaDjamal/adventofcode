...
class Position {
    ...
    parseInput(input){
        ...
        let number = parseInt(input.join(''));
        ...
        if(direction === 'L' || direction === 'R') {
            ...
            if(direction === 'D') number *= -1;
            this.y += number;
        }
    }
}

let origin = new Position("U0", 0, 0);

wire1.push(origin);
wire2.push(origin);

for(let i = 0; i < input1.length; i++) {
    wire1.push(new Position(input1[i], wire1[i].x, wire1[i].y));
}
for(let j = 0; j < input2.length; j++) {
    wire2.push(new Position(input2[j], wire2[j].x, wire2[j].y));
}
console.log(wire2);